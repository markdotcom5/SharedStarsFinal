...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        if (!moduleData) {
            console.log(
                '❌ Simulation Training Module Not Found in MongoDB—Returning Default Data'
            );
            moduleData = {
        ...
        res.status(500).json({
            success: false,
            error: 'Failed to fetch Simulation Training Module',
        });
    }
});
...
router.get('/mission-control/:userId', authenticate, async (req, res) => {
    try {
        const userMissions = await Simulation.find({ userId: req.params.userId }).sort({
            completionTime: -1,
        });
        res.status(200).json(userMissions);
    } catch (error) {
        res.status(500).json({
            error: 'Failed to retrieve mission control data',
            details: error.message,
        });
    }
});
