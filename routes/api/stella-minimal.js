 ...
 */
// ✅ Step 4: Intelligent Router `/guidance`
router.post('/guidance', async (req, res) => {
  const { userId, question } = req.body;

  if (!userId || !question) {
    return res.status(400).json({ success: false, message: 'userId and question required' });
  }

  // Try internal SLL first
  let responseContent = await fetchFromSLL(question, userId);
  ...
  if (!responseContent) {
    ...
    const recentMessages = recentConversations.map(msg => 
      `${msg.fromUser ? 'User' : 'STELLA'}: ${msg.content}`
    ).join("\n");

    // Fetch user progress data (updated query)
    ...
    const progressInfo = `
    USER PROGRESS:
    - Active Mission: ${userProgress?.physicalTraining.activeMission || 'Not started'}
    - Completed Missions: ${userProgress?.physicalTraining.completedMissions || 0}
    - Training Credits: ${userProgress?.credits?.total || 0}
    `;

    // Fetch Stella Knowledge insights
    const StellaKnowledge = require('../../models/StellaKnowledge');
    const insights = await StellaKnowledge.find({}).limit(10);
    const learnedInsights = insights.map(insight => `- ${insight.summary}`).join("\n");

    const learningPrompt = `
    STELLA's Learned Insights:
    ${learnedInsights}
    `;
    const systemPrompt = `
    You are STELLA, a personalized AI space-training assistant.
    
    ${learningPrompt} // Include this line
    
    Provide direct, actionable, concise answers to user questions.
    `;
    
    // Enhanced system prompt
    const enhancedSystemPrompt = `
    You are STELLA, an advanced AI for space training.
    ...
    Respond directly to the user's question, explicitly referencing recent interactions, learned insights, or progress where relevant.
    `;
    ...
    try {
      ...
      responseContent = openaiRes.choices[0].message.content;

      // Learn: store new response into SLL
      ...
      return res.status(500).json({ success: false, message: 'STELLA encountered an error' });
    }
  }
  ...
  res.json({ success: true, guidance: { message: responseContent } });
});
