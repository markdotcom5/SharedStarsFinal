...
async function updateMissionProgress(req, res) {
  try {
    ...
    const updatedMission = await Mission.findByIdAndUpdate(req.params.missionId, { 
      $set: { 'progressTracking.stepsCompleted': stepsCompleted }
    }, { new: true });

    res.status(200).json(updatedMission);
  } catch (error) {
    res.status(500).json({ error: 'Failed to update mission progress', details: error.message });
  }
}
