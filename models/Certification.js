...
certificationSchema.methods.calculateAIMetrics = async function () {
    ...
    if (
        this.progress &&
        typeof this.progress.percentageComplete === 'number' &&
        ...
        aiAnalysis &&
        aiAnalysis.metrics &&
        typeof aiAnalysis.metrics.completionRate === 'number'
    ) {
        // Otherwise, if AI analysis is available, use its computed completion rate.
        this.aiMetrics.predictedCompletion = new Date(
            Date.now() + (100 - aiAnalysis.metrics.completionRate) * 86400000
        );
    } else {
    ...
    return {
        ...
        improvementAreas:
            (aiAnalysis && aiAnalysis.metrics && aiAnalysis.metrics.skillLevels) || [],
    };
};
