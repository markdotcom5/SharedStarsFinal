...
leaderboardSchema.methods.updateRank = async function () {
    const leaderboard = await this.constructor
...
leaderboardSchema.methods.updateRank = async function () {
    const strategy = await AIController.generateLeaderboardStrategy({
        user: this.userId,
        leaderboardData: await this.constructor.find({ category: this.category }),
    });
