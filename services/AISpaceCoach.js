...
class AISpaceCoach extends EventEmitter {
        ...
        try {
            ...
            const completion = await openai.chat.completions.create({
                ...
                messages: [
                    ...
                    {
                        role: 'user',
                        content: `Generate coaching suggestions based on: ${JSON.stringify(context)}`,
                    },
                ],
                temperature: 0.7,
            });
    ...
    async processAssessmentAnswer(userId, questionIndex, answer, language = 'en') {
        try {
    ...
    async trackProgress(userId, data) {
        try {
            ...
            if (streakUpdated) {
                await this.checkStreakAchievements(userId, userMetrics.streakData.currentStreak);
            }
